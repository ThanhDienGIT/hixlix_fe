...
function EditStaff(props) {
    ...
    const CallAPIByIdStaff = async (id) => {
        await Axios.get('getStaffByID/' + id).then(res => {
            setUser(res.data)
    ...
    React.useEffect(() => {
        CallAPIByIdStaff(props.idstaff)
    }, [props.idstaff]);
    return (
        <Dialog
            open={props.open}
            ...
            <DialogContent>
                <Box display={'flex'} flexDirection={'column'} padding={1}>
                    ...
                    <FormControl sx={{ marginTop: 2 }}>
                        ...
                        <Select
                            value={Number(user.TRANGTHAI_NV)}
                            name={'TRANGTHAI_NV'}
                            onChange={(e) => { onChangeInput(e) }}
                        >
