...
import AddBOSetting from 'pages/component/AddBOSetting'
import EditBOSetting from 'pages/component/EditBOSetting'
import DialogDeleteBOSetting from 'pages/component/DialogDeleteBOSetting'


function BOSettingManagement() {

    const { token } = useContext(TokenContext);
    token
    const [data, setData] = useState([])
    const [DialogBO, setDialogBO] = useState(false)
    const [dialogSuccess, setDialogSuccess] = useState(false)
    const [dialogError, setDialogError] = useState(false)
    const [dialogEdit, setdialogEdit] = useState(false)
    const [idBO, setidBO] = useState(0)
    ...
    const CallAPIPage = async () => {
        ...
        await instance.get(`get_danhsachcauhinhBO/${rowPage}?page=${page}`).then(res => {
    ...
    const handleSearch = async () => {
        ...
        await instance.post(`search-BO-setting/${rowPage}?page=${page}`, objectSend)
            .then((res) => {
                console.log(res)
                setData(res.data.dsbost.data)
                setMaxPage(res.data.dsbost.last_page)
                setAlloption(res.data.dsbost.data)
                setSearchStatus(1)
                setLoading(false)
                const newStartIndex = (page - 1) * rowPage + 1;
                setStartIndex(newStartIndex);
    ...
    return (
        <ComponentSkeleton>
            <MainCard title="DANH MỤC KHẢ NĂNG CHUYỂN BO">
                {loadingInitial ?
                    ...
                    <>
                        <Box display={'flex'} sx={{ alignItems: 'center', marginBottom: 1, flexWrap: "wrap" }} justifyContent={'space-between'}>
                            <Box display="flex" alignItems="center">
                                <Autocomplete
                                    ...
                                    id="free-solo-2-demo"
                                    disableClearable
                                    options={alloption.map((option) => option.TEN_BO)}
                        ...
                        /> */}
                            ...
                            <Box display={'flex'} marginTop={1} sx={screenWidth > 720 ? "" : { width: '100%' }} >
                                ...
                                <Button size="small" sx={{ display: 'flex', mr: 1, width: 150 }} color={'primary'} variant="contained" onClick={openDialogBO}>
                                            ...
                                            <TableRow key={index}>
                                                ...
                                                <TableCell>
                                                    <Tooltip title="Cập nhật">
                                                        <IconButton>
                                                            <DriveFileRenameOutlineIcon color='info' onClick={() => { openDialogEditKH(ele.ID_BO) }} />
                                                        </IconButton>
                                                    </Tooltip>
                                                    <Tooltip title="Xóa">
                                                        <IconButton>
                                                            <DeleteIcon color='error' onClick={() => { openDialogError(ele.ID_BO) }} />
                                                        </IconButton>
                                                    </Tooltip>
                                                </TableCell>
                                            </TableRow>
            ...
            <DialogDeleteBOSetting
                open={dialogError}
                handleClose={closeDialogError}
                idBO={idBO}
                callApi={CallAPIProps}
                content={"Xác nhận xóa tiêu chí chuyển BO này?"}
            />

        </ComponentSkeleton>
    )
}
export default BOSettingManagement
