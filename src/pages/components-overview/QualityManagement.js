...
function QualityManagement() {
    ...
    const CallAPIPage = async () => {
        ...
        await instance.get(`get_danhsachchatluong/${rowPage}?page=${page}`).then(res => {
    ...
    const handleSearch = async () => {
        ...
        await instance.post(`search-quality/${rowPage}?page=${page}`, objectSend)
            .then((res) => {
                console.log(res)
                setData(res.data.dsqlt.data)
                setMaxPage(res.data.dsqlt.last_page)
                setAlloption(res.data.dsqlt.data)
                setSearchStatus(1)
                setLoading(false)
                const newStartIndex = (page - 1) * rowPage + 1;
                setStartIndex(newStartIndex);
    ...
    return (
        <ComponentSkeleton>
            <MainCard title="DANH MỤC CHẤT LƯỢNG DỊCH VỤ">
                {loadingInitial ?
                        ...
                        <Box display={'flex'} sx={{ alignItems: 'center', marginBottom: 1, flexWrap: "wrap" }} justifyContent={'space-between'}>
                            <Box display="flex" alignItems="center">
                                <Autocomplete
                                    ...
                                    id="free-solo-2-demo"
                                    disableClearable
                                    options={alloption.map((option) => option.TEN_CHATLUONG)}
                            ...
                            <Table size='small'>
                                <TableHead sx={{ backgroundColor: '#0099ff' }} >
                                    <TableRow>
                                        ...
                                        <TableCell sx={{ color: 'white' }}> Tên chất lượng </TableCell>
                                    ...
                                    {data.map((ele, index) => {
                                        return (
                                            <TableRow key={index}>
                                                ...
                                                <TableCell>
                                                    <Tooltip title="Cập nhật chất lượng dịch vụ">
                                                        <IconButton>
                                                            <DriveFileRenameOutlineIcon color='info' onClick={() => { openDialogEditKH(ele.ID_CHATLUONG) }} />
                                                        </IconButton>
                                                    </Tooltip>
                                                    <Tooltip title="Xóa chất lượng dịch vụ">
                                                        <IconButton>
                                                            <DeleteIcon color='error' onClick={() => { openDialogError(ele.ID_CHATLUONG) }} />
                                                        </IconButton>
                                                    </Tooltip>
                                                </TableCell>
                                            </TableRow>
            ...
            <DialogDeleteQuality
                ...
                idquality={idquality}
                callApi={CallAPIProps}
                content={"Xác nhận xóa chất lượng dịch vụ này?"}
            />
